...
var require_make_client = __commonJS({
  "node_modules/@grpc/grpc-js/build/src/make-client.js"(exports2) {
    ...
    function loadPackageDefinition2(packageDef2) {
      const result = {};
      for (const serviceFqn in packageDef2) {
        if (Object.prototype.hasOwnProperty.call(packageDef2, serviceFqn)) {
          const service = packageDef2[serviceFqn];
...
var require_namespace = __commonJS({
  "node_modules/protobufjs/src/namespace.js"(exports2, module2) {
    ...
    Namespace.prototype.define = function define2(path2, json) {
      ...
      if (path2.length > util.recursionLimit)
...
var require_server_credentials = __commonJS({
  "node_modules/@grpc/grpc-js/build/src/server-credentials.js"(exports2) {
    ...
    exports2.ServerCredentials = ServerCredentials2;
    var InsecureServerCredentials = class _InsecureServerCredentials extends ServerCredentials2 {
      constructor() {
        super(null);
      }
      _getSettings() {
    ...
    var SecureServerCredentials = class _SecureServerCredentials extends ServerCredentials2 {
    ...
    var CertificateProviderServerCredentials = class _CertificateProviderServerCredentials extends ServerCredentials2 {
    ...
    var InterceptorServerCredentials = class _InterceptorServerCredentials extends ServerCredentials2 {
    ...
    var Server2 = (() => {
      ...
      return _a = class Server {
        ...
        experimentalCreateConnectionInjectorWithChannelzRef(credentials2, channelzRef, ownsChannelzRef = false) {
          ...
          const server2 = this.createHttp2Server(credentials2);
...
var grpc3 = __toESM(require_src3());

// packages/shared/src/loader.ts
var import_path = __toESM(require("path"));
var grpc = __toESM(require_src3());
var protoLoader = __toESM(require_src4());
var PROTO_PATH = import_path.default.resolve(
  __dirname,
...
var migrationProto = proto;

// packages/coordinator/src/worker-client.ts
var grpc2 = __toESM(require_src3());
var CHUNK_SIZE = 64 * 1024;
function createWorkerClient(address) {
  return new migrationProto.migration.WorkerService(
    address,
    grpc2.credentials.createInsecure()
  );
}
async function sendMigrationToWorker(workerAddress, migrationId, filename, content) {
  return new Promise((resolve) => {
    const client = createWorkerClient(workerAddress);
    const call = client.applyMigration(
      (err, result) => {
        client.close();
        if (err) {
...
async function listMigrationsFromWorker(workerAddress) {
  return new Promise((resolve) => {
    const client = createWorkerClient(workerAddress);
    const empty = {};
...
// packages/coordinator/src/coordinator.service.ts
function getWorkerAddresses() {
  const raw = process.env.WORKER_URLS ?? "localhost:50052";
  return raw.split(",").map((s) => s.trim()).filter(Boolean);
}
var coordinatorServiceImpl = {
  uploadMigration(call, callback) {
    const chunks = [];
    let migrationId = "";
    let filename = "";
    call.on("data", (chunk) => {
    ...
    call.on("end", async () => {
      ...
      const results = await Promise.all(
        workerAddresses.map(
          (addr) => sendMigrationToWorker(addr, migrationId, filename, content)
        )
      );
      const allSuccess = results.every((r) => r.success);
      const summary = results.map((r) => `  ${r.worker_address}: ${r.success ? "OK" : "ERRO"} \u2014 ${r.message}`).join("\n");
      console.log(`[coordinator] Resultado:
${summary}`);
      callback(null, {
        success: allSuccess,
        message: allSuccess ? "Migration aplicada em todos os workers." : "Um ou mais workers falharam.",
        workers: results
      });
    });
    call.on("error", (err) => {
  ...
  async listMigrations(_call, callback) {
    const workerAddresses = getWorkerAddresses();
...
server.bindAsync(ADDRESS, grpc3.ServerCredentials.createInsecure(), (err, port) => {
  if (err) {
