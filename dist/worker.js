...
var require_namespace = __commonJS({
  "node_modules/protobufjs/src/namespace.js"(exports2, module2) {
    ...
    Namespace.prototype.define = function define2(path3, json) {
      ...
      if (path3.length > util.recursionLimit)
...
var DATA_DIR = import_path2.default.resolve(
  __dirname,
...
var workerId = process.env.WORKER_ID ?? "worker-default";
var dbPath = import_path2.default.join(DATA_DIR, `${workerId}.db`);
...
`);
function isMigrationApplied(migrationId) {
  const row = db.prepare("SELECT 1 FROM _migrations WHERE migration_id = ?").get(migrationId);
  return row !== void 0;
}
...
var workerServiceImpl = {
  applyMigration(call, callback) {
    const chunks = [];
    let migrationId = "";
    let filename = "";
    call.on("data", (chunk) => {
    ...
    call.on("end", () => {
      try {
        if (isMigrationApplied(migrationId)) {
          callback(null, {
            success: false,
            message: `Migration '${migrationId}' j\xE1 foi aplicada anteriormente.`
          });
          return;
        }
...
server.bindAsync(ADDRESS, grpc2.ServerCredentials.createInsecure(), (err, port) => {
  if (err) {
